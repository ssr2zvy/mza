...
| `protocol` | required | Identifies the bundling contract user must implement. See [cargo-bundler-v0.1.0](docs/protocols/cargo-bundler-v0.1.0.md) when another Rust crate is cross-compiled into the final bundle executable, or [command-bundler-v0.1.0](docs/protocols/command-bundler-v0.1.0.md) when a host-run Rust adapter invokes an external bundling system. Also used to clarify file paths | `protocol = "cargo-bundler-v0.1.0"` |
